...
def claim_hash(agent, i):
    return "0x" + hashlib.sha256(f"exp6:{agent}:{i}".encode()).hexdigest()


def main():
    anvil = subprocess.Popen(["anvil", "--port", "8549", "--silent"],
    ...
    try:
        ...
        print("anvil(8549) up", flush=True)
        ...
        bv = deploy("BondedValidator", [token, idreg, valreg, judge,
                                       str(10**18), "60"])
        lens = deploy("ReputationLens", [valreg, bv])
        print(f"IdentityRegistry={idreg}\nValidationRegistry={valreg}\nBondedValidator={bv}\nReputationLens={lens}", flush=True)
        ...
        s_credit = sig("creditScore(uint256)")
        s_abrate = sig("abstainRateBp(uint256)")
        s_reqbp = sig("requiredBondBp(uint256)")
        ...
        def lens_read(aid):
            raw = call(lens, enc(s_credit, [("u", aid)]))[2:]
            score, answered = int(raw[:64], 16), int(raw[64:128], 16)
            abrate = int(call(lens, enc(s_abrate, [("u", aid)]))[2:][:64], 16)
            reqbp = int(call(lens, enc(s_reqbp, [("u", aid)]))[2:][:64], 16)
            return {"score_neutral": score, "answered": answered,
                    "abstain_rate_bp": abrate, "required_bond_bp": reqbp}
        ...
        lensdata = {k: lens_read(v) for k, v in agent_ids.items()}
        k1 = (lensdata["extractor"]["score_neutral"] >= 95
              and lensdata["hallucinator"]["score_neutral"] <= 60)
        k2 = (lensdata["honest"]["required_bond_bp"] <= lensdata["extractor"]["required_bond_bp"]
              < lensdata["hallucinator"]["required_bond_bp"])
        verdict = {"K1_abstain_neutral_pass": bool(k1), "K2_bond_ordering_pass": bool(k2)}
        results = {
            "n_problems": N_PROBLEMS, "tau": TAU,
            "final_bonds": {k: v[-1] for k, v in traj.items()},
            "moves": counts,
            "onchain_reputation_naive": {k: {"validations": c, "avg_score": a}
                                   for k, (c, a) in reputation.items()},
            "reputation_lens": lensdata,
            "verdict": verdict,
            "contracts": {"IdentityRegistry": idreg, "ValidationRegistry": valreg,
                          "BondedValidator": bv},
        }
        json.dump({"results": results, "trajectories": traj},
                  open("/Users/yubbi/iis-lab/exp6/out/results.json", "w"), indent=2)
        ...
        fig.tight_layout()
        fig.savefig("/Users/yubbi/iis-lab/exp6/out/bonded_validation.png", dpi=140)
