...
ERC-8004 gives AI agents on-chain identity, reputation, and a validation registry — but the spec deliberately leaves staking, slashing, and incentives to "specific validation protocols" built on top. Most designs emerging in that slot bond the *validator* (staked re-execution). We are building the complementary missing piece: a protocol that bonds the *speaker*. An agent cannot make a claim unless free bond is locked against it; a claim proven wrong is slashed; an honest "I don't know" (abstention) releases the bond without loss. Accountability attaches at the moment of utterance, not at audit time.
...
We are asking for a small grant to turn this prototype into a public good for the 8004 ecosystem: (1) a hardened, audited-quality open-source reference implementation of the Bonded Validation protocol with a specified interface others can implement; (2) public testnet deployment against the canonical 8004 registries with a reproducible benchmark harness; (3) a research report on abstention economics for bonded agents (how slash ratios should set abstention thresholds), with all datasets and code released. Everything will be MIT/Apache licensed.
