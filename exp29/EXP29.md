...
## 14. Phase 2 결과 — 온체인 무응답 요청자 슬라이스 (Anvil, 2026-09-03)

**범위:** Phase 2 전체(3종 적대 요청자 × 모드 P/B)가 아니라 **무응답 요청자** 슬라이스만(K3a·K3b·K3c·K4a·K4d). 위조 서명(K4b)·이동표적(K4c)은 판정기에 적대적 입력을 실제로 주입해야 하는 별도 구현이 필요해 미착수로 남긴다. 재현: `python3 run_exp29_phase2_unresponsive.py 200`(컨트랙트 무수정, BondedValidatorV3/BondedJudgePanelV3 그대로 배포).

**시나리오:** ask_policy(δ=1 낙관적 신뢰, κ=0.05, B=5/R=1)로 200스레드 결정 → 100건 ask, 100건 즉시 speak/abstain. ask 100건에서 상대는 끝까지 침묵(δ 실측치 아님 — 에이전트 신뢰와 무관하게 환경이 강제).

**K3(b) 모드 P: 흔적 0 — PASS.** ask 하지만 답이 안 오면(R9 모드 P) 온체인 트랜잭션 자체가 0건(100/100, nonce 변화 없음 직접 확인) — 명료화가 오프체인 계산으로 끝났을 때 온체인 흔적이 진짜로 없다.

**K3(a)·K3(b) 모드 B — 문자 그대로는 KILL, 원인은 블록 타임스탬프 양자화(설계 결함 아님).** ask+무응답 100스레드의 잠금시간이 [86400, 86401]초(문턱 ≤86400) — 절반가량이 1초 초과. **그런데 질문을 아예 안 한 대조군 100스레드(같은 컨트랙트, 같은 절차)도 정확히 같은 범위 [86400, 86401]초** — `windowOpen := block.timestamp < claimedAt + W`(컨트랙트 원문, 엄격 부등호가 아니라 `<`)라 이론적 최소 잠금은 정확히 W이지만, anvil 자동 채굴이 실제로는 블록마다 타임스탬프를 최소 1초 단위로만 진행시켜 종종 W+1에 걸린다. **명료화가 원인이 아니라는 증거는 대조군이 통계적으로 구분 불가능하다는 것 자체다** — 원 K3(a) 문구("질문 없는 발화와 동일")의 취지(같은 코드 경로·같은 분포)는 성립하지만, 절대 문턱 "≤W"는 이산 블록시간을 반영 못 한 사전등록 문구의 정밀도 문제로 판단한다. **문자 그대로는 KILL 처리하고, 재등록 시 "≤ W + 1블록"으로 정정 필요 — 규칙 자체는 수정하지 않는다(사전등록 원칙).**

**K4(a) 토큰 보존 — PASS.** LabToken totalSupply 실행 전후 완전 동일(10,000×10^18, wei 단위까지). 명료화로 인한 토큰 이동 0.
